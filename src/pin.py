    ...
    try:
        for _ in range(iterations):
            for fr in frames:
                ...
                lines = fr.split(b"\n")
                # save cursor (DECSC), paint glyphs at absolute positions,
                # then restore cursor (DECRC) so a concurrent log writer is
                # not disturbed. The whole frame is one write+flush.
                buf = bytearray()
                buf += ESC + b"7"
